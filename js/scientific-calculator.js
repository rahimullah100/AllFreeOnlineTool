    ...
    function evaluateExpression() {
        ...
        try {
            let expr = display.value
                .replace(/sin\(([^)]+)\)/g, 'dSin($1)')
                .replace(/cos\(([^)]+)\)/g, 'dCos($1)')
                .replace(/tan\(([^)]+)\)/g, 'dTan($1)')
                .replace(/log\(([^)]+)\)/g, 'dLog($1)')
                .replace(/ln\(([^)]+)\)/g, 'dLn($1)')
                .replace(/π/g, Math.PI)
                .replace(/e/g, Math.E)
                .replace(/÷/g, '/')
            ...
            function dSin(x) { return Math.sin(Number(x) * Math.PI / 180); }
            function dCos(x) { return Math.cos(Number(x) * Math.PI / 180); }
            function dTan(x) { return Math.tan(Number(x) * Math.PI / 180); }
            function dLog(x) { return Math.log10(Number(x)); }
            function dLn(x) { return Math.log(Number(x)); }

            let result;
            if (typeof math !== 'undefined' && math.evaluate) {
                // Use math.js for robust parsing if available (math.js supports ^ natively)
                result = math.evaluate(expr, {dSin, dCos, dTan, dLog, dLn});
            } else {
                // Fallback: replace ^ with Math.pow(a, b)
                expr = expr.replace(/([\d.]+|\([^()]+\))\s*\^\s*([\d.]+|\([^()]+\))/g, 'Math.pow($1,$2)');
                result = Function('dSin', 'dCos', 'dTan', 'dLog', 'dLn', 'Math', 'return ' + expr)(dSin, dCos, dTan, dLog, dLn, Math);
            }
            display.value = (typeof result === 'number' && Number.isFinite(result)) ? result.toString() : 'Error';
