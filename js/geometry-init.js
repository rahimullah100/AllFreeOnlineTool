function showGeometryInputs() {
    console.log('[Geometry] showGeometryInputs called');
...
function reinitGeometryCalculator() {
    const btn = document.getElementById('calculate-geometry');
    if (btn) {
            ...
            try {
                ...
                if (resultElement) {
                    ...
                    if (result.area !== undefined) {
                        resultHTML += `<div class="result-item"><span class="result-label">Area:</span><span class="result-value">${result.area.toFixed(6)}</span></div>`;
                    }
                    if (result.perimeter !== undefined) {
                        resultHTML += `<div class="result-item"><span class="result-label">Perimeter:</span><span class="result-value">${result.perimeter.toFixed(6)}</span></div>`;
                    }
                    if (result.volume !== undefined) {
                        resultHTML += `<div class="result-item"><span class="result-label">Volume:</span><span class="result-value">${result.volume.toFixed(6)}</span></div>`;
                    }
                    if (result.surfaceArea !== undefined) {
                        resultHTML += `<div class="result-item"><span class="result-label">Surface Area:</span><span class="result-value">${result.surfaceArea.toFixed(6)}</span></div>`;
                    }
                    resultHTML += '</div>';
...
if (document.readyState === 'complete' || document.readyState === 'interactive') {
    reinitGeometryCalculator();
} else {
    window.addEventListener('DOMContentLoaded', reinitGeometryCalculator);
} 